...
public class InputBoxView extends FrameLayout {
    ...
    public InputBoxView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.InputBoxView);
            codeCount = ta.getInteger(R.styleable.InputBoxView_codeCount, DEFAULT_CODECOUNT);
            encryption = ta.getBoolean(R.styleable.InputBoxView_encryption, true);
            softKeyboardAuto = ta.getBoolean(R.styleable.InputBoxView_softKeyboardAuto, true);
        }
        if (codeCount < 1) {
            codeCount = 1;
    ...
    private void init() {
        LayoutInflater.from(getContext()).inflate(R.layout.layout_input_box_view, this);
