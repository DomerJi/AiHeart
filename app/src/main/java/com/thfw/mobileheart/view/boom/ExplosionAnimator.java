 ...
 */
package com.thfw.mobileheart.view.boom;
...
public class ExplosionAnimator extends ValueAnimator {
    ...
    public ExplosionAnimator(View container, List<Bitmap> bitmaps, Rect bound) {
        mPaint = new Paint();
        mBound = new Rect(bound);
        int size = bitmaps.size();
        mParticles = new Particle[bitmaps.size()];
        Random random = new Random(System.currentTimeMillis());

        for (int i = 0; i < size; i++) {
            mParticles[i] = generateParticle(Color.GREEN, random);
            mParticles[i].bitmap = bitmaps.get(i);
        }
    ...
    public boolean draw(Canvas canvas) {
        ...
        for (Particle particle : mParticles) {
            ...
            if (particle.bitmap != null) {
                stop = true;
                canvas.drawBitmap(particle.bitmap, particle.cx, particle.cy, null);
            } else {
        ...
        if (!stop || (float) getAnimatedValue() < 0.78) {
